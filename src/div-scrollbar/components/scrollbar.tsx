...
import type { Axis, ScrollState, UserStyleConfig } from "../types";
import { resolveScrollbarStyles } from "../scrollbar/view/styling/resolveScrollbarStyles";
import { AXIS_CONFIG } from "../scrollbar/constants";
import { ScrollbarController } from "../scrollbar/core/scrollbarController";
...
export function Scrollbar({
  ...
  const styles = useMemo(
    () =>
      resolveScrollbarStyles({
        axis,
        customStyle,
        isInteracting,
      }),
    [axis, customStyle, isInteracting],
  );
  ...
  useEffect(() => {
    ...
    controllerRef.current = new ScrollbarController({
      el: scrollAreaRef.current,
      cfg,
      onUpdate: (current, max) => {
        if (!layoutRef.current || !thumbRef.current || !scrollAreaRef.current)
          return;
        ...
        const trackSize = layoutRef.current[cfg.trackSize];

        let thumbSize = (visible / total) * trackSize;

        const min = styles.quickStyle.minimumSizePx;
        thumbSize = Math.max(min, thumbSize);

        thumbRef.current.style[cfg.sizeStyleProp] = `${thumbSize}px`;

        // debug
        console.log("Thumb calc:", {
          visible,
          total,
          trackSize,
          thumbSizeBeforeMax: (visible / total) * trackSize,
          finalThumbSize: Math.max(min, thumbSize),
        });
        const movableRange = trackSize - thumbSize;
        const ratio = max === 0 ? 0 : current / max;
        const offset = movableRange * ratio;

        thumbRef.current.style.transform = cfg.transform(offset);

        setScrollState((prev) => {
          if (prev.current === current && prev.max === max) return prev;
          return { current, max };
        });
      },
  ...
  }, [cfg, scrollAreaRef, styles]);

  useEffect(() => {
    const scrollArea = scrollAreaRef.current;
    if (!scrollArea || !controllerRef.current) return;

    const handleWheel = (e: WheelEvent) => {
      controllerRef.current!.handleWheel(e.deltaY);
    };
  ...
  return (
    ...
    >
      <div className="track" ref={TrackRef} style={styles.trackStyle}>
        <div
          ref={thumbRef}
          role="scrollbar"
          tabIndex={0}
...
//   mergeHoverstyle,
//   mergeStyleConfig,
// } from "../scrollbar/view/styling/mergeStyles";
// import {
//   AXIS_CONFIG,
//   //  DRAG_THRESHOLD
...
//   getAxisStyle,
//   getHoverStyle,
// } from "../scrollbar/view/styling/getStyles";
